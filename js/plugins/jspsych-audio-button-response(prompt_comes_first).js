...
jsPsych.plugins["audio-button-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // Putting prompt at the stop

    var html = ""
    html+=  '<div style="text-align: center; font-size:24px;">请判断</div><br><br>';

    // display images
    if (!trial.mirror) {
      if (trial.image !== null) {
      html +='<img src="'+trial.image+'" id="jspsych-audio-button-response-stimulus"/><br>';
    }
    } else {
      if (trial.image !== null) {
        html +='<img style="transform: scaleX(-1);" src="'+trial.image+'" id="jspsych-audio-button-response-stimulus"/><br>';
      }
    }
