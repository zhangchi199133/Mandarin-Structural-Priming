...
jsPsych.plugins["image-audio-response"] = (function() {
        ...
        parameters: {
            ...
            recording_light: {
                ...
                default: '<div id="jspsych-image-audio-response-light" '+
                    ...
                    'width: 25px; height: 25px; border-radius: 25px; margin: 10px auto; '+
                    'display: block;"></div>',
            ...
            word: {
                ...
                pretty_name: 'Word',
                default: null,
                description: 'Any word here will be displayed above the image.'
            },
            prompt: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Prompt',
            ...
            mirror: {
                type: jsPsych.plugins.parameterType.BOOL,
                pretty_name: 'Image mirroring',
                default: false,
                description: 'If true, the stimulus image will flip.'
        }
    }
  };
    plugin.trial = function(display_element, trial) {
        ...
        html+=  '<div style="text-align: center; font-size:24px;">请描述</div><br>';

        if (trial.word !== null) {
            html +=  '<div style="text-align: center; font-size:28px;"><b>'+trial.word+'</b></div><br><br>';
        }

        // add images
        if (!trial.mirror) {
          html +='<img src="'+trial.stimulus+'" id="jspsych-image-audio-response-stimulus"/><br>';
        } else {
            html +='<img style="transform: scaleX(-1);" src="'+trial.stimulus+'" id="jspsych-image-audio-response-stimulus"/><br>';
        }

        // add prompt if there is one
        if (trial.prompt !== null) {
            html += '<br><div style="text-align: center; font-size:28px;"><b>'+trial.prompt+'</b></div><br>';
        }
        ...
        html += '<div id="jspsych-image-audio-response-buttons"><button id="jspsych-image-audio-response-okay" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">确认</button><button id="jspsych-image-audio-response-rerecord" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">重录</button></div>';
